...
import type { Match, TeamIdentifier } from "./types";
...
Seeds("First and second seed are spread apart", () => {
  ...
  assert.ok(
    [bracket16.winners[0]!.upperTeam, bracket16.winners[0]!.lowerTeam].includes(
      ...
      [
        bracket16.winners[0]!.upperTeam,
        bracket16.winners[0]!.lowerTeam,
      ].includes(2)
  );
  ...
  assert.ok(
    [lastMatchWithATeam!.upperTeam, lastMatchWithATeam!.lowerTeam].includes(
      1
    ) ||
