...
type Weapon = ElementType<typeof weapons>;
type SubWeapon =
  | "Splat Bomb"
  | "Suction Bomb"
  | "Burst Bomb"
...
function buildToAP(
  ...
  build.headAbilities.forEach((ability, index) => {
    if (ability !== "UNKNOWN") {
...
export default function useAbilityEffects({
  ...
  function calculateISM(amount: number) {
    ...
    if (mInkConsume) {
      const title =
        weapon.includes("Splatling") || weapon.includes("Nautilus")
          ? t`Full charges per ink tank`
          : t`Shots per ink tank`;
    ...
    if (mInkConsumeRepeat && mInkConsumeRepeat !== mInkConsume) {
      toReturn.push({
        title: t`Shots per ink tank (autofire mode)`,
        effect: `${parseFloat(
    ...
    if (mFullChargeInkConsume) {
      toReturn.push({
        title: t`Fully charged shots per ink tank`,
        effect: `${parseFloat(
    ...
    if (mMinChargeInkConsume) {
      toReturn.push({
        title: t`Tap shots per ink tank`,
        effect: `${parseFloat(
    ...
    if (
      ...
      toReturn.push({
        title: t`Ground swings per ink tank`,
        effect: `${parseFloat(
      ...
      toReturn.push({
        title: t`Jumping swings per ink tank`,
        effect: `${parseFloat(
          (1 / (mInkConsumeSplashJump * effect[0])).toFixed(2)
        )}`,
        effectFromMax: effect[1],
    ...
    if (mSideStepInkConsume) {
      toReturn.push({
        title: t`Dodge roll ink consumption`,
        effect: `${parseFloat(
    ...
    if (mInkConsumeUmbrella) {
      toReturn.push({
        title: t`Brella shield launch ink consumption`,
        effect: `${parseFloat(
  ...
  function calculateISS(amount: number) {
    ...
    return [
      {
        title: `${i18n._(subWeapon)} ${t`ink consumption`}`,
        effect: `${percentage}% ${t`of ink tank`}`,
        effectFromMax: effect[1],
  ...
  function calculateREC(amount: number) {
    ...
    return [
      {
        title: t`Ink tank recovery from empty to full (squid form)`,
        effect: `${Math.ceil(effectSquid[0] * tank)} ${t`frames`} (${parseFloat(
  ...
  function calculateSSU(amount: number) {
    ...
    return [
      {
        title: t`Swim speed`,
        effect: `${parseFloat(speed.toFixed(2))} ${t`distance units / frame`}`,
        effectFromMax: effect[1],
  ...
  function calculateSS(amountBeforeRP: number) {
    ...
    toReturn.push({
      title: t`Special lost when killed`,
      effect: `${parseFloat(
        ((1.0 - specialLost) * 100).toFixed(2)
      )}% ${t`of the charge`}`,
      effectFromMax,
    ...
    if (weaponData[weapon].Special === "Splashdown") {
      ...
      toReturn.push({
        title: t`Special lost when killed mid-Splashdown`,
        effect: `${parseFloat(
          ((1.0 - lost) * 100).toFixed(2)
        )}% ${t`of the charge`}`,
  ...
  function calculateSPU(amount: number) {
    ...
    const specialWeaponTranslated = i18n._(specialWeapon);
    ...
    if (
      ...
      const effect = getEffect(highMidLow, amount);
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`duration`}`,
        effect: `${Math.ceil(effect[0])} ${t`frames`} (${parseFloat(
        ...
        info:
          specialWeapon === "Inkjet"
            ? t`Special Power Up also increases Inkjet's shots' painting and blast radius`
            : undefined,
        ap: amount,
    ...
    if (specialWeapon === "Tenta Missiles") {
      ...
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`reticle size`}`,
        effect: `${parseFloat(
      ...
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`ink coverage`}`,
        effect: `${parseFloat(
    ...
    if (specialWeapon === "Splashdown") {
      ...
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`180dmg hitbox size`}`,
        effect: `${parseFloat(
      ...
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`70dmg hitbox size`}`,
        effect: `${parseFloat(
        ...
        ability: "SPU" as Ability,
        info: t`55dmg hitbox can't be increased with Special Power Up so the total radius of the special doesn't change`,
        ap: amount,
        getEffect: (ap: number) =>
          parseFloat(
            (
    ...
    if (specialWeapon === "Ink Armor") {
      ...
      const effect = getEffect(highMidLow, amount);
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`activation time`}`,
        effect: `${Math.ceil(effect[0])} ${t`frames`} (${parseFloat(
          (Math.ceil(effect[0]) / 60).toFixed(2)
        )} ${t`seconds`})`,
        effectFromMax: effect[1],
    ...
    if (specialWeapon === "Baller") {
      ...
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`durability`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
      ...
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`55dmg explosion hitbox size`}`,
        effect: `${parseFloat(
    ...
    if (specialWeapon === "Bubble Blower") {
      ...
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`bubble size`}`,
        effect: `${parseFloat(
      ...
      const effectAtZeroHit = getEffect(highMidLowHit, 0);
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`explosion hitbox`}`,
        effect: `${parseFloat(
          ((effectHit[0] / effectAtZeroHit[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effectHit[1],
    ...
    if (specialWeapon === "Booyah Bomb") {
      ...
      const effectAtZero = getEffect(highMidLow, 0);
      toReturn.push({
        title: `${specialWeaponTranslated} ${t`autocharge speed`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
  ...
  function calculateQR(amount: number) {
    ...
    return [
      {
        title: `${t`Quick Respawn`} ${t`time`}`,
        effect: `${totalFrames} ${t`frames`} (${parseFloat(
        ...
        info: t`Quick Respawn activates when enemy kills you twice without you getting a kill in between`,
        ap: amount,
        getEffect: (ap: number) =>
          Math.ceil(
            150 +
  ...
  function calculateQSJ(amount: number) {
    ...
    return [
      {
        title: `${t`Quick Super Jump`} ${t`time`} ${t`(on the ground)`}`,
        effect: `${Math.ceil(effectTame[0])} ${t`frames`} (${parseFloat(
      ...
      {
        title: `${t`Quick Super Jump`} ${t`time`} ${t`(in the air)`}`,
        effect: `${Math.ceil(effectMove[0])} ${t`frames`} (${parseFloat(
  ...
  function calculateBRU(amount: number) {
    ...
    const subWeaponTranslated = i18n._(subWeapon);
    ...
    if (
      ...
      toReturn.push({
        title: `${subWeaponTranslated} ${t`range and velocity`}`,
        effect: `${parseFloat(
    ...
    if (subWeapon === "Sprinkler") {
      ...
      toReturn.push({
        title: `${subWeaponTranslated} ${t`full-power phase duration`}`,
        effect: `${Math.ceil(effectFirst[0])} ${t`frames`} (${parseFloat(
      ...
      toReturn.push({
        title: `${subWeaponTranslated} ${t`mid-phase duration`}`,
        effect: `${Math.ceil(effectSecond[0])} ${t`frames`} (${parseFloat(
    ...
    if (["Point Sensor", "Ink Mine"].includes(subWeapon)) {
      ...
      toReturn.push({
        title: `${subWeaponTranslated} ${t`tracking duration`}`,
        effect: `${Math.ceil(effect[0])} ${t`frames`} (${parseFloat(
          (Math.ceil(effect[0]) / 60).toFixed(2)
        )} ${t`seconds`})`,
        effectFromMax: effect[1],
    ...
    if (subWeapon === "Ink Mine") {
      ...
      toReturn.push({
        title: `${subWeaponTranslated} ${t`tracking range`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (subWeapon === "Splash Wall") {
      ...
      toReturn.push({
        title: `${subWeaponTranslated} ${t`durability`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (subWeapon === "Squid Beakon") {
      ...
      toReturn.push({
        ...
        info: t`When jumping to Sub Power Up boosted beakons QSJ AP bonus is applied on top of any existing QSJ the jumper has. 57AP can't be exceeded. Value shown is the bonus if the user of Beakon has 0AP invested in QSJ.`,
      });
    }
  ...
  function calculateRES(amount: number) {
    ...
    return [
      {
        title: t`Frames before taking damage from enemy ink`,
        ...
        info: t`Ink Resistance Up also allows you to jump higher and strafe faster while shooting in enemy ink`,
      },
      {
        title: t`Damage taken in enemy ink`,
        effect: `${parseFloat(
      ...
      {
        title: t`Limit on the damage enemy ink can deal on you`,
        effect: `${parseFloat(
      ...
      {
        title: t`Run speed in enemy ink`,
        effect: `${parseFloat(
          effectVel[0].toFixed(2)
        )} ${t`distance units / frame`}`,
  ...
  function calculateBDU(amount: number) {
    ...
    return [
      {
        title: t`Sub Weapon damage (indirect)`,
        ...
        info: t`Bomb Defense Up DX also lessens the damage of a direct bomb hit but it will never make a bomb not kill that would have dealt over 100dmg without any of the ability on`,
      },
      {
        title: t`Special Weapon damage (indirect)`,
        ...
        info: t`Tenta Missiles, Inkjet, Splashdown, Baller, Bubble Blower, Booyah Bomb & Ultra Stamp generate damage lessened by Bomb Defense Up DX. OHKO's are unaffected`,
      },
      {
        title: t`Base tracking time (Point Sensor)`,
        effect: `${Math.ceil(
      ...
      {
        title: t`Base tracking time (Ink Mine)`,
        effect: `${Math.ceil(
          mineFrames * effectMine[0]
        )} ${t`frames`} (${parseFloat(
  ...
  function calculateMPU(amount: number) {
    ...
    const weaponTranslated = i18n._(weapon);
    ...
    if (
      ...
      toReturn.push({
        title: `${weaponTranslated} ${t`bullet ink coverage`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      const effectAtZero = getEffect(highMidLow, 0);
      toReturn.push({
        title: `${weaponTranslated} ${t`bullet spread (not jumping)`}`,
    ...
    if (
      ...
      const effectAtZero = getEffect(highMidLow, 0);
      toReturn.push({
        title: `${weaponTranslated} ${t`bullet spread (in the air)`}`,
        effect: `${parseFloat(effect[0].toFixed(2))} (${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%)`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      const effectAtMax = getEffect(highMidLow, MAX_AP);
      toReturn.push({
        title: `${weaponTranslated} ${t`range and bullet velocity`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      toReturn.push({
        title: `${weaponTranslated} ${t`70dmg hitbox portion of the total explosion hitbox`}`,
    ...
    if (weapon.includes("Rapid")) {
      ...
      toReturn.push({
        title: `${weaponTranslated} ${t`bullet explosion hitbox size`}`,
    ...
    if (
      ...
      const effectAtMax = getEffect(highMidLow, MAX_AP);
      toReturn.push({
        title: `${weaponTranslated} ${t`bullet explosion paint`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      const effectAtMax = getEffect(highMidLow, MAX_AP);
      toReturn.push({
        title: `${weaponTranslated} ${t`movement speed holding trigger down`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      const effectAtMax = getEffect(highMidLow, MAX_AP);
      toReturn.push({
        title: `${weaponTranslated} ${t`ink trail width`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      const effectAtMax = getEffect(highMidLow, MAX_AP);
      toReturn.push({
        title: `${weaponTranslated} ${t`fully charged shot range`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      const effectAtMax = getEffect(highMidLow, MAX_AP);
      toReturn.push({
        title: `${weaponTranslated} ${t`ink coverage`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      if (!weapon.includes("Bamboozler")) {
        toReturn.push({
          title: `${weaponTranslated} ${t`damage`}`,
          effect: `${parseFloat(
            ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
          )}%`,
          effectFromMax: effect[1],
        ...
        toReturn.push({
          title: `${weaponTranslated} ${t`damage per fully charged shot`}`,
          effect: `${damage}`,
          effectFromMax: effect[1],
    ...
    if (
      ...
      toReturn.push({
        ...
        info: weapon.includes("Sloshing")
          ? t`Ink coverage bonus is only applied to the circle at the end of the shot`
          : undefined,
      });
    }
    ...
    if (weapon === "Explosher" || weapon === "Custom Explosher") {
      ...
      toReturn.push({
        ...
        info: t`Ink coverage bonus is only applied to the line before the impact`,
      });
    }
    ...
    if (
      ...
      const damageMinStr = baseDamageMin !== -1 ? `${damageMin} - ` : "";
      toReturn.push({
        title: `${weaponTranslated} ${t`damage per shot (fully charged)`}`,
        effect: `${damageMinStr}${damageMax}`,
        effectFromMax: effect[1],
        effectFromMaxActual: damageMax,
        ability: "MPU" as Ability,
    ...
    if (
      ...
      const effectAtZero = getEffect(highMidLow, 0);
      toReturn.push({
        title: `${weaponTranslated} ${t`shield recharge time`}`,
        effect: `${Math.ceil(effect[0])} ${t`frames`} (${parseFloat(
          (Math.ceil(effect[0]) / 60).toFixed(2)
        )} ${t`seconds`})`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      const effectAtMax = getEffect(highMidLow, MAX_AP);
      toReturn.push({
        title: `${weaponTranslated} ${t`shield durability`}`,
        effect: `${parseFloat(
          ((effect[0] / effectAtZero[0]) * 100).toFixed(2)
        )}%`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      toReturn.push({
        title: `${weaponTranslated} ${t`full charge shooting duration`}`,
        effect: `${total} ${t`frames`} (${parseFloat(
          (total / 60).toFixed(2)
        )} ${t`seconds`})`,
        effectFromMax: effect[1],
    ...
    if (
      ...
      toReturn.push({
        ...
        getEffect: (ap: number) =>
          parseFloat(getEffect(highMidLow, ap)[0].toFixed(3)),
        info: t`Slosher has weird physics so we haven't fully deciphered this parameter.`,
      });
  ...
  const abilityFunctions: Partial<
    Record<string, (amount: number) => Explanation[]>
  > = {
    ISM: calculateISM,
    ISS: calculateISS,
    REC: calculateREC,
  ...
  useEffect(() => {
    if (!weapon) return;
    const AP = buildToAP(buildsAbilities, bonusAp, lde);
  ...
  }, [JSON.stringify(buildsAbilities), weapon, bonusAp, lde]);
